...
def view_inventory():
    ...
    global stock_tree, inventory_window
    ...
    inventory_window.title("Items and Stock")
    inventory_window.geometry("630x400")
    inventory_window.configure(bg="#626262", padx=28, pady=28)
    inventory_window.iconbitmap("ui_images\\main_icon.ico")

    inventoryLabel = tk.Label(inventory_window, text="P R O D U C T   I N V E N T O R Y", bg="#626262", fg="#FFFFFF", font="Raavi 22 bold")
    inventoryLabel.pack(pady=(0,16))

    inventorybutframe = tk.Frame(inventory_window, height=60, width=350, bg="#626262")
    inventorybutframe.pack()

    updatestock = tk.Button(inventorybutframe, text="Update Stock", command=update_item_window, width=15, height=1, font="Raavi 11 bold")
    updatestock.grid(row=0, column=0)

    removeItem = tk.Button(inventorybutframe, text="Remove Item", command=remove_item_window, width=15, height=1, font="Raavi 11 bold")
    removeItem.grid(row=0, column=1, padx=16)

    addItem = tk.Button(inventorybutframe, text="Add Item", command=add_item_window, width=15, height=1, font="Raavi 11 bold")
    addItem.grid(row=0, column=2)
    
    # Create labels to display items and stock
    stock_tree = ttk.Treeview(inventory_window, columns=("Item", "Stock"), height=12)
    stock_tree.heading("#0", text="Product Code")
    stock_tree.heading("Item", text="Item Name")
    stock_tree.heading("Stock", text="Stock")
    stock_tree.pack(expand=True, fill=tk.BOTH, pady=(12,0))

    stock_tree.column("#0", width=50, anchor=tk.CENTER)  # Set the width of the first column
    stock_tree.column("Item", width=150)  # Set the width of the "Name" column
    stock_tree.column("Stock", width=50, anchor=tk.CENTER)    # Set the width of the "Age" column

    style = ttk.Style()
    style.configure("Treeview", font=("Helvetica", 10), rowheight=30, bg='#DDD1B8', fg="#C4643E")
    style.configure("Treeview.Heading", font="Hussar 11 bold", bg='#DDD1B8', fg="#C4643E")
    
    # Insert items and stock into the Treeview
    for idx, item_data in enumerate(data):
        for item, stock in item_data.items():
            stock_tree.insert("", "end", text=idx+1, values=(item, stock))
            idx += 1
...
def update_item():
    ...
    if item and stock.isdigit():  # Check if both item and stock are provided and stock is a digit
        with open("product_inventory.json", 'r+') as file:
            data = json.load(file)
            if item in data[0]:  # Check if item exists in inventory
                data[0][item] = int(stock)  # Update stock value for the item as an integer
                ...
                print(f"Stock for {item} updated to {stock}.")
            else:
                print(f"Item '{item}' not found in the inventory.")

        item_entry.delete(0, tk.END)  # Clear item entry
        stock_entry.delete(0, tk.END)  # Clear stock entry
...
def update_item_window():
    ...
    update_window.config(padx=15, pady=15, background="#fcf7f0")

    item_label = tk.Label(update_window, text="Item:", bg="#fcf7f0")
    item_label.grid(row=0, column=0, padx=5, pady=5)

    stock_label = tk.Label(update_window, text="Stock:", bg="#fcf7f0")
    stock_label.grid(row=1, column=0, padx=5, pady=5)

    global item_entry, stock_entry
    ...
    fetch_button = tk.Button(update_window, text="Fetch Stock", command=fetch_stock)
    fetch_button.grid(row=2, column=0, padx=5, pady=5)

    update_button = tk.Button(update_window, text="Update Stock", command=update_item)
...
def fetch_stock():
    ...
    with open("product_inventory.json", 'r') as file:
        ...
        if item in data[0]:  # Check if item exists in inventory
            ...
            stock_entry.insert(0, data[0][item])  # Insert current stock value into the entry
        else:
            stock_entry.delete(0, tk.END)
            print(f"Item '{item}' not found in the inventory.")
...
def removeitem():
    ...
    selected_item = remove_entry.get()

    # Check if the selected item exists in the data
    if selected_item in data[0]:
        del data[0][selected_item]  # Remove the selected item from the dictionary
        ...
        print(f"Item '{selected_item}' not found in the inventory.")


def remove_item_window():
    global remove_entry, remove_item
    remove_item = tk.Toplevel(root)
    remove_item.geometry("259x150")
    remove_item.config(padx=15, pady=15, background="#fcf7f0")

    remove_entry = tk.Entry(remove_item)
    remove_entry.pack()

    removebutton = tk.Button(remove_item, text="Remove Item", command=removeitem)
    removebutton.pack()


def add_item_window():
    global add_item_entry, add_stock_entry, add_item

    add_item = tk.Toplevel(root)
    add_item.geometry("250x150")
    add_item.config(padx=15, pady=15, background="#fcf7f0")

    item_label = tk.Label(add_item, text="Item:", bg="#fcf7f0")
    item_label.grid(row=0, column=0, padx=5, pady=5)

    stock_label = tk.Label(add_item, text="Stock:", bg="#fcf7f0")
    stock_label.grid(row=1, column=0, padx=5, pady=5)

    add_item_entry = tk.Entry(add_item)
    add_item_entry.grid(row=0, column=1, padx=5, pady=5)

    add_stock_entry = tk.Entry(add_item)
    add_stock_entry.grid(row=1, column=1, padx=5, pady=5)

    add_button = tk.Button(add_item, text="Add Item", command=add_item_stock)
...
def add_item_stock():
    ...
    newItem = add_item_entry.get()
    stock = int(add_stock_entry.get())

    if newItem in data[0]:

        messagebox.showerror("Invalid Item", f"Item '{newItem}' already exists!")

    else:

        data[0][newItem] = stock

        messagebox.showinfo("Item Added", f"Item '{newItem}' has been succesfully added.")
...
def show_login_history():
    history_window = None  # Initialize history_window variable
    ...
    try:
        ...
        # Create a Toplevel window to display login history
        history_window = Toplevel(root)
        history_window.title("Login History")
        history_window.geometry("600x250")
        history_window.iconbitmap("ui_images\\main_icon.ico")

        style = ttk.Style()
        

        # Display login history in the Toplevel window
        tree = ttk.Treeview(history_window, columns=("Index", "Username", "Time"), show="headings")

        # Set column headings
        tree.heading("Index", text="Login Index")
        tree.heading("Username", text="Username")
        tree.heading("Time", text="Time")

        tree.column("Index", width=15, anchor=tk.CENTER)
        tree.column("Username", width=100, anchor=tk.CENTER)
        tree.column("Time", width=100, anchor=tk.CENTER)

        style.configure("Treeview", font=("Helvetica", 10), rowheight=30, background='#fcf7f0', fg="#C4643E")
        style.configure("Treeview.Heading", font="Helvetica 12 bold", background='#fcf7f0', fg="#C4643E")
        style.layout("Treeview", [('Treeview.treearea', {'sticky ': 'nswe'})])  

        # Display login history in the Treeview
        total_records = len(login_history)
        for i, login_record in enumerate(login_history, 1):
            reversed_index = total_records - i + 1
            username = login_record["username"]
            login_time = login_record["time"]
            tree.insert("", "end", values=(reversed_index, username, login_time))

        # Pack the Treeview widget
        tree.pack(fill="both", expand=True)
    except (FileNotFoundError, json.JSONDecodeError):
        pass  # Do nothing if there's no login history or the file is empty

    if history_window is None:
        # Create a Toplevel window to display a message
        history_window = Toplevel(root)
        history_window.title("Login History")

        # Display a message indicating no login history available
        Label(history_window, text="No login history available.").pack()


def check_stock_before_checkout():
    # Load the stock data from the JSON file
    with open('product_inventory.json', 'r') as file:
        stock_data = json.load(file)[0]
    
    # Check each item in the cart
    for item_id in tree.get_children():
        item_name = tree.item(item_id, 'values')[0]
        cart_quantity = int(tree.item(item_id, 'values')[1])

        # Find the item in the stock data
        if item_name in stock_data:
            stock_quantity = int(stock_data[item_name])  # Ensure the stock quantity is treated as an integer
            if cart_quantity > stock_quantity:
                messagebox.showerror("Insufficient Stock", f"Not enough stock for {item_name}. Available: {stock_quantity}, Required: {cart_quantity}")
                return False
        else:
            messagebox.showerror("Item Not Found", f"{item_name} is not found in the inventory.")
            return False

    # If all items have sufficient stock, return True to proceed with checkout
    return True

def proceed_checkout():
    if check_stock_before_checkout():
        # Proceed with checkout process if stock is sufficient
        CheckEntries()
...
cartbutton.grid(row=0, column=4, sticky=tk.NSEW, padx=23, pady=10)
...
historybutton.grid(row=0, column=3, sticky=tk.NSEW, padx=23, pady=10)

inventory_image = tk.PhotoImage(file="ui_images\\inventory.png")
inventorybutton = tk.Button(HeaderFrame, command=view_inventory, bg='#DDD1B8', border=0, image=inventory_image, width=70)
inventorybutton.grid(row=0, column=2, sticky=tk.NSEW, padx=23, pady=10)

loginhistory_img = tk.PhotoImage(file='ui_images\\admin history.png')
loginhistory = tk.Button(HeaderFrame, command=show_login_history, bg='#DDD1B8', border=0, width=70, image=loginhistory_img)
loginhistory.grid(row=0, column=1, sticky=tk.NSEW, padx=23, pady=10)
...
checkout = tk.Button(CartButtonFrame, text="Checkout", bg='#DDD1B8', font=("Hussar 12 bold"), fg="#C4643E", command=proceed_checkout)
